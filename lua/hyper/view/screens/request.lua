...
res:add_keymap({"n", "U", function()
  Menu.entry(State.get_state("url"), {
    title = "URL",
    row = 0,
...
res:add_keymap({"n", "P", function()
  Menu.entry(State.get_state("query_params"), {
...
res:add_keymap({"n", "B", function()
  if Util.is_body_method(State.get_state("method")) then
    Menu.entry(State.get_state("body"), {
      title = "Body",
      row = 1,
      col = Config.layout_config.col_width,
      width = width - 2 * Config.layout_config.col_width,
...
res:add_keymap({"n", "H", function()
  Menu.entry(State.get_state("headers"), {
    title = "Headers",
    row = 2,
    col = 0,
    width = 40,
...
res:add_keymap({"n", "R", function()
  local state = State.get_state()
  local env = Env
  print("make_request::env:", vim.inspect(env))
